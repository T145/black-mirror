...
Black Mirror works by taking blacklist or whitelist listings from `data/sources.json`.
...
Now go make a Pull Request with a new entry in `data/sources.json`!
Don't worry about the other "sources" files or where to place the entry:
the extra "sources" files are autogenerated, and `data/sources.json` gets sorted alphabetically.
...
A redundant source would be any source that has its own entry in `data/sources.json` and is mostly or fully included in another list.
