...
domain_entries=$(printf "%'d" $(cat black_domain.txt | wc -l))
domain_size=$(du -h black_domain.txt | gawk -F'\t' '{ print $1 }')

ipv4_entries=$(printf "%'d" $(($(cat black_ipv4.txt | wc -l) - $domain_entries)))
ipv4_size=$(du -h black_ipv4.txt | gawk -F'\t' '{ print $1 }')

ipv6_entries=$(printf "%'d" $(($(cat black_ipv6.txt | wc -l) - $domain_entries)))
